...
$sql_ent = "
SELECT id, name, level, completename AS cname
FROM `glpi_entities`
WHERE id IN (".$ents.")
";
...
$arr_data = array();
while ($row_res = $DB->fetch_assoc($result_ent)) {

	$arr_data[] = '{id: "'.$row_res['id'].'", text: "'.$row_res['name'].'", level: "'.$row_res['level'].'"}';
	$arr_data2[] = $row_res['id'];
}
...
//$sons = getSonsOf('glpi_entities', $arr_data2[1]);
//print_r($sons);
 
//$pais = getAncestorsOf('glpi_entities', $arr_data2[0]); 
//print_r($pais);

//$tree = getTreeForItem('glpi_entities', $arr_data2[1]);  
//print_r($tree);

//echo getEntityName(28);

/*foreach ($arr_data2 as $e) {
	echo getEntityLevel($e);
}*/

//Entity::showSelector($_POST['target'], "activeentity");


//getChildren($arr_data2[26]);
//print_r($nodes);
//$nodes = '';

//getChildren($arr_data2[27]);
//print_r($nodes);


//$sons = getSonsOf('glpi_entities', $nodes[0]['id']);
$sons = getSonsOf('glpi_entities', $arr_data2[26]);
//print_r($sons);
$nodes = '';

foreach ($arr_data2 as $s) {
	
	$sons = getSonsOf('glpi_entities', $s);	
	
//	if(count($sons) > 1 ) {
	
		//echo $nodes[0]['children'];
		//$node = $sons[2];
		
//		getChildren($s);		
//		print_r($nodes);
		
//echo $s." ".getEntityName($s)." ".getEntityLevel($s)."\n";

	if($s != 0) {

		if(count($sons) > 1) {
			//print_r(array_unique($sons));
			//$s3 = unique_multidim_array($sons,'text');
			$s3 = super_unique($sons);
			
			foreach ($s3 as $v) {
				//$sons2 = getChildren($s);
				//print_r($s2);
								
				echo $v." ".getEntityName($v)." ".getEntityLevel($v)."\n";
				//$nodes = "";		
			}
		}		
	}		
...
foreach ($arr_data2 as $d) {

//	print_r(getChildren($d));
...
//Entity::showSelector($CFG_GLPI['root_doc']."/front/central.php", "activeentity");
									?>
									<select id="entities" name="sel_ent"></select>
								</td>
							</tr>
