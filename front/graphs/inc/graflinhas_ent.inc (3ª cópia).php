...
if($interval >= "31") {
	...
	while ($row_result = $DB->fetch_assoc($resultd)) {
	   ...
	   $queryb = "
		...
		$resultb = $DB->query($queryb) or die('erro m');
		$row_result2 = $DB->fetch_assoc($resultb);
	
		//$v_row_result = $row_result['day'];
		//if($row_result2['nb'] != '') {
		//	$arr_bak[$v_row_result] = $row_result2['nb'];
			$arr_bak[] = $row_result2['nb'];
		//}
		//else {
		//	$arr_bak[$v_row_result] = 0;
		//}
	}	
	//$arr_opened = $arr_grfm;
	$back_ini = array_sum(array_values($arr_bak));
}
...
for ($i=2; $i < ($conta); $i++) {
	$j = $i-1;
	$arr_bk[] = $arr_bk[$j] + ($quant_o[$j] - $quant_c[$j]);	
}
